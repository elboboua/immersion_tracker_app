...
const languageRequestRoutes = require("./language-request.routes");
...
router.get("/count-hours", async (req, res) => {
  let result = await knex("log")
    .select(knex.raw("sum(time) as minutes"))
    .where("deleted", "=", "0");
...
router.get("/count-languages", async (req, res) => {
  let result = await knex("log").select(
    knex.raw("count(distinct language_id) as count")
  );
  res.send(result[0]);
});
...
router.get("/get-latest-logs", async (req, res) => {
  let result = await knex("log")
    ...
    .select(
      knex.raw("user.username, log.name as activity, language.name as language")
    )
    .where("user.username", "<>", '""')
    .where("log.deleted", "=", "0")
    .orderBy("log.id", "desc")
